...
dataframe = dataframe.drop(dataframe[dataframe['_value'] > 100].index)
...
get_order_arima_model = pm.auto_arima(dataframe)
...
_data_frame = pd.DataFrame(prediction, columns=['humivaluepredicted'])
...
_write_client.write(bucket, my_org, record = _data_frame, data_frame_measurement_name = 'PredictionHumidity')
